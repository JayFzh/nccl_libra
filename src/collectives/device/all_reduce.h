...
__device__ void ncclAllReduceRingKernel(struct CollectiveArgs* args) {
  // printf("In ncclAllReduceRingKernel the empty is:%d\n",args->empty);
  ...
  if(args->empty == 1){
    long int CLOCK_RATE = 1695000*1000;  /* modify for different device */
    clock_t t0 = clock64();
    // printf("clock1: %ld\n",t0);
    clock_t t1 = t0;
    while ((t1 - t0)*8.7/10 <CLOCK_RATE){
      t1 = clock64();
    }
    // int sec = (t1-t0)/CLOCK_RATE;
