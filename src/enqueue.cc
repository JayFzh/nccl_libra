...
ncclResult_t ncclBarrierEnqueue(struct ncclComm* comm) {
  ...
  if (comm->launchMode == ncclComm::GROUP && (comm->groupCudaStream || comm->userStream == NULL)) {
    ...
    if (comm->userStream != params->stream && comm->allreduce != 1) {
      printf("in 184\n");
...
static ncclResult_t getAlgoInfo(struct ncclInfo* info) {
  ...
  // lyz -alloc
  if (info->block_num > 0) info->nChannels = info->block_num;
  if (info->thread_num > 0) {
    if (info->protocol == NCCL_PROTO_SIMPLE) info->nThreads = info->thread_num + WARP_SIZE;
    else info->nThreads = info->thread_num;
  }
  return ncclSuccess;
}
...
static ncclResult_t computeColl(struct ncclInfo* info /* input */, struct ncclColl* coll, struct ncclProxyArgs* proxyArgs /* output */) {
  ...
  coll->args.empty = info->empty;
  if(coll->args.empty == 1){
    printf("In computeColl the empty is:%d\n",coll->args.empty);
  }
...
ncclResult_t ncclSaveKernel(struct ncclInfo* info) {
  ...
  //info->nThreads = 128;
  // lyz - alloc
  info->comm->allreduce = info->allreduce;
  if(info->comm->allreduce == 1){

  }
  else{
    info->comm->allreduce = 0;
  }
  info->comm->myParams->blockDim.x = std::max<unsigned>(info->comm->myParams->blockDim.x, info->nThreads);
  INFO(NCCL_INIT, "Using %d threads for each block\n", info->comm->myParams->blockDim.x);

  int nChannels = info->coll == ncclCollSendRecv ? 1 : coll.args.coll.nChannels;
  INFO(NCCL_INIT, "Using %d channels in total\n", nChannels);
...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  ...
  if (ncclAsyncMode()) {
    ...
    // printf("fzh-debug:in enque end check\n");
    return ncclSuccess;
  }
}
